...
* Check the final results against the checklist from the google cert
...
Yes: All variable names were investigated manually and modified as required: see details in below section.
...
rm(df_types_target, df_types_after)
```
...
```{r clean_misspellings, eval=FALSE}
for(df_name in df_names) {
  print(df_name)
  glimpse(get(df_name))
}
...
Given that the data was not entered manually by the user, there's no way for me to manually check the correctness of all of the numeric values included in the data set. The values will instead be checked against pre-determined limits to verify that they fall within realistic ranges given the types of data presented.
...
Sources for ranges:
  * Fat %: American Journal of Clinical Nutrition via Forbes: https://pubmed.ncbi.nlm.nih.gov/10966886/
...
## Duplicates: Did you check for and remove duplicates in the data?
