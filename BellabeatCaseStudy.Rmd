...
As discussed above, the exercise-related "Intensity" variable is a numerical factor with no defined range: the limits of the range and meaning of the values must be determined before the analysis can proceed. I'll start by determining the range of values present in the source Intensity data:
...
As discussed above, the sleep-related "Value" variable is a numerical factor with no defined range: the limits of the range and meaning of the values must be determined before the analysis can proceed. As with exercise intensity, I start by determining the range of values present in the data:
...
* Removed missing data
  * The "bodycomp_src_logs_wide" table was found to be missing the majority of the entries for the body-fat percentage variable: this variable will not be used in the analysis.
...
```{r analyse-util-fns}
# Declare generic functions for analysis
...
```{r analyse-plot-histo-pareto-fn}
...
```{r analyse-plot-time-coefficients-fn}
...
```{r analyse-plot-wide-data-to-stacked-bar-fn}
...
```{r analyse-plot-scatter-with-LOBF-fn}
...
viz_all_intensity_over_time <- ggplot(intensity_time_coeffs) +
  ...
  labs(title = "Coefficient of Active Hours with Time, by Intensity",
...
```{r analyse-viz-night-usage-sleep-logs}
...
```{r analyse-viz-all-heartrate-data}
...
```{r analyse-viz-heartrate-polling-variance}
# Visualise heartrate polling variance
...
```{r analyse-viz-night-usage-heartrate}
...
```{r analyse-viz-sleep-logs-vs-heartrate-logs}
...
```{r analyse-viz-sleep-over-time-bruteforce, eval=FALSE}
...
```{r analyse-viz-sleep-quantity-over-time}
...
```{r analyse-viz-sleep-quality-over-time}
...
```{r analyse-viz-daytime-heartrate-usage}
# Plot histogram showing who logged what fraction of their nights
...
```{r analyse-viz-intensity-levels}
...
* Add green Cumulative Percentage of Users line to plot_histo_pareto()
* Standardise Mean/Median captions and 
...
```{r share-viz-avg-time-by-intensity, echo=FALSE, fig.dim = c(6, 4), fig.cap = "AYO THIS A FIG.CAP"}
print(viz_avg_time_by_intensity)
```
...
```{r share-viz-active-hrs-per-week, echo=FALSE}
print(viz_active_hrs_per_week)
```
...
```{r share-viz-very-active-proportion, echo=FALSE}
print(viz_very_active_proportion)
```
...
```{r share-viz-very-active-vs-total-active, echo=FALSE}
print(viz_very_active_vs_total_active)
```

> Users get around 7500 steps in per day on average

```{r share-viz-mean-daily-steps, echo=FALSE}
print(viz_mean_daily_steps)
```
    
> Walking is much more popular than running:

```{r share-viz-walking, echo=FALSE}
print(viz_walking_speed_frequency)
```

> Exercise intensity did not increase over time

```{r share-viz-all-intensity-over-time, echo=FALSE}
print(viz_all_intensity_over_time)
```
...
```{r share-viz-distance-vs-activity, echo=FALSE}
print(viz_distance_vs_activity)
```
