...
At this point, I would ideally cross-reference the calorie data with body composition data to see if the higher-calorie IDs are associated with users with a higher BMI or weight, which would imply a higher base energy requirement and therefore less exercise. However, only 8 of the 33 users have logged body composition, versus all 33 users who logged calories, so this would not be comprehensive.

I can use the Exercise Intensity metric to get a better idea of who's burning more calories. To get a baseline view, I'll visualise each ID's breakdown of the four Intensity types:

TODO: Find weekly averages for intensity: this may give a better idea of exercise habits, since people are unlikely to exercise every day
TODO: Generate normalised versions of the stacked bar graphs

```{r average_intensities_by_id}
# For each ID, seperate daily averages for the four intensity levels
intensity_order = c("very_active", "fairly_active", "lightly_active", "sedentary")
mean_daily_intensities <- intensity_sum_days_wide %>%
  group_by(id) %>%
...
mean_daily_intensities_long <- mean_daily_intensities %>%
  tidyr::gather(key = "intensity", value = "mean_minutes", -id) %>%
  mutate(intensity = factor(intensity, levels = rev(intensity_order)))

ggplot(mean_daily_intensities_long, aes(x= id, y= mean_minutes, fill= intensity)) +
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

```{r average_intensities_by_id_weekly}
# For each ID, seperate daily averages for the four intensity levels
intensity_order = c("very_active", "fairly_active", "lightly_active")
mean_weekly_intensities <- intensity_sum_days_wide %>%
  group_by(id) %>%
  summarize("lightly_active" = mean(minutes_lightly_active)*7/60,
            "fairly_active" = mean(minutes_fairly_active)*7/60,
            "very_active" = mean(minutes_very_active)*7/60)
  
mean_weekly_intensities_long <- mean_weekly_intensities %>%
  tidyr::gather(key = "intensity", value = "mean_hours", -id) %>%
  mutate(intensity = factor(intensity, levels = rev(intensity_order)))

ggplot(mean_weekly_intensities_long, aes(x= id, y= mean_hours, fill= intensity)) +
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
Caveats:
* The averages don't add up to full days, so its possible people aren't tracking all of their exercising.
...
TODO: Display gradient of line-of-best-fit as well as R^2 value
...
grid.arrange(plot_cal_vs_non_sedentary, plot_cal_vs_active, plot_cal_vs_very_active, ncol = 3)
```


Calories burned is shown to correlate only loosely with non-sedentary time, withR^2 = 0.041, but more tightly with active and very-active time, with R^2 = 0.32 and R^2 = 0.4, respectively.

Visual inspection of the charts confirms a fairly-obvious theory: more time spent doing higher-intensity activities correlates with a rapid increase in calories burned. 

UNSCIENTIFIC: There's also a decent spread of calories-burned even at the lower ranges of active time, which is what you'd expect from a diverse group of mostly-sedentary people.

TODO: Histogram the weekly activity hours
Overall finding: most people aren't doing more than 0-5 hours moderate/heavy exercise per week. No more than 4 people are doing more than 10 hours per week.
