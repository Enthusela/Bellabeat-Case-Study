...
The following data analysis case study was completed as my capstone project for the Google Data Analytics Professional Certificate. It is organised to reflect the overall approach to data analysis advanced in that course, and so is split into six overarching steps: "Ask", "Prepare", "Process", "Analyse", "Share", and "Act". The process taken to establish the business question and find and process relevant data for analysis is detailed in the "Ask", "Prepare", and "Process" steps; the analysis and my preliminary findings are detailed in the "Analyse" step; and my final findings from the data and recommendations to the stakeholders are detailed in the "Share" and "Act" steps, respectively.
...
* Sufficient power: enough to handle large data sets, e.g. FitBit data tables with over two million observations
* Functions for data manipulation, e.g. loading, processing and combining data sets
...
* A straightforward learning process and user interface: spreadsheet tools, for example, typically have a single "Add Chart" tool under which all of their powerful charting options can be found; contrast this with learning to download, enable, and finally use the ggplot2 R library
...
Note: Another obvious candidate, Python, was not considered for this analysis: while it shares most of the features, advantages, and disadvantages of R, I'm already familiar with Python and wanted to use this case study to familiarise myself with R instead.

Based on this assessment, I decided on R as the best single tool for this analysis. R lacks the straightforward operation of spreadsheet tools, and I will need to learn libraries and programming techniques as I do my analysis, but this is acceptable given my prior experience with other programming languages like Python. 
...
The full R code used for this section is included in Appendix B.
...
* Data sampling rate is encoded inconsistently: compare "heartrate_seconds_merged.csv" to "daySleep_merged.csv"
* Feature name is often not first in the file name, affecting file sort operations
...
For example, "sleepDay_merged.csv" will be renamed to "sleep_sum_days_wide.csv". This format removes the ambiguities in the original file names, ensures all required information on the contents of the data is included, and automatically groups data by feature.
...
Two variables in the data set, "Intensity" (for exercise) and "Value" (for sleep), are numerical factors with no defined range. For these factors, I couldn't tell from the data alone whether all possible values that a FitBit can record are present, or what real-world states the numeric values represented. Analysis of the factor variables confirmed they followed the mappings listed below:
...
```{r vrfy-compare-sleep-value-data}
...
The following broad data-processing steps were taken:
...
These steps are further detailed in the sections below. The full R code used for this section is included in Appendix C.
...
```{r process-declare-var-update-fns}
...
```{r process-update-variable-names}
...
```{r process-update-nonfactor-variable-types}
...
```{r process-update-factor-variable-types}
...
```{r process-trim-whitespace-variables}
# Trim variable names
...
```{r process-trim-whitespace-data}
# Trim data
...
* Step counts make sense (e.g. < 20000)
* Calories are within normal range
* Heart rates are less than 200

```{r process-validate-all-numeric}
...
```{r process-validate-specific-numeric}
...
* Calories: 21 records showed caloric burns of over 4000 calories. All but one of those were associated with very high levels of activity: the outlier showed only 30 minutes total as "Very Active" or "Lightly Active", compared to 120+ minutes for all other records. It does show 15km distance covered that day, which is nearly a third of a marathon, so for now it makes enough sense to keep it in, and I will analyse it further after the processing stage.
...
There were a range of features to consider when comparing the FitBit devices in the dataset to the Bellabeat Ivy. Of these, I chose to focus this analysis on those features that were both available on the Ivy and available in the data set, either directly or by a suitable proxy data. This approach allowed me to focus my recommendations to the marketing team on those features that I could draw reliable conclusions on. The full R code used for this section is included in Appendix D.
...
```{r analyse-all-walking-over-time}
# Get correlation coefficents with time for all walking intensities
...
```{r analyse-plot-all-walking-over-time}
# Plot correlation coefficents with time for all walking intensities
...
```{r analyse-plot-mean-daily-step-counts}
...
```{r analyse-plot-mean-daily-steps-vs-time}
...
```{r analyse-plot-distance-vs-intensity}
...
The following wristband FitBits are included in the data set:
...
This meant that even choosing the median value of 5 seconds could result in an inferred "time logged" value that was out by a factor of 4.
...
viz_nighttime_heartrate_usage <- plot_histo_pareto(nights_logged_by_heart_rate$nights_logged_pct, bin_width = 0.05) +
  gglayer_theme +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Percentage of Nights Logged In Bed - HR Users Only",
       subtitle = "Source: Heart Rate Data")
print(viz_nighttime_heartrate_usage)
...
* Given the ubiquity of sleep-tracking functionality among FitBit devices, these findings indicated that the majority of users were not making use of this function.
* Out of all users, 73% (24 users) did not log any heart rate-based sleep logs.
...
The sleep tracking feature is core to much of the functionality of the Ivy. The [existing marketing material](https://bellabeat.com/ivy/features/) claims that "Ivy monitors your sleep patterns and knows exactly how your night is going to affect the next day." The sleep logging is also factored into other functions like the Readiness Score.

In order to better analyse how users were currently making use of this function, I combined my previous analysis on overnight wear with additional analysis on existing user sleep patterns.
...
# Visualise hours spent asleep
viz_average_hours_asleep <- plot_histo_pareto(sleep_sum_quant_qual_mean$hours_total_mean, bin_width = 1) +
  gglayer_theme +
  labs(title = "Average Hours Asleep",
       x = "Hours Asleep")
print(viz_average_hours_asleep)
...
# Visualise hours in bed spent asleep
viz_average_pct_asleep <- plot_histo_pareto(sleep_sum_quant_qual_mean$pct_asleep_mean, bin_width = 0.05) +
  gglayer_theme +
  labs(title = "Percentage of Sleep Logs Spent Asleep",
       x= "% of Time")
print(viz_average_pct_asleep)
...
* Time spent Awake is almost nil, with 88% of users logging less than 5% of time asleep. This is likely due to the programming of the FitBit devices, which feature an automatic sleep logging function that begins logging when it detects the user may be asleep, based on movement and heart rate data
...
viz_sleep_quantity_vs_time <- get_time_coefficients_plot(sleep_sum_quant_qual$id,
...
# Plot histogram showing who logged what fraction of their nights
viz_daytime_heartrate_usage <- plot_histo_pareto(periods_logged$periods_pct, bin_width = 0.05) +
  gglayer_theme +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Percentage of Daytime Periods Logged",
       subtitle = "Source: Heart Rate Data")
print(viz_daytime_heartrate_usage)
...
For this feature I analysed what percentage of various users' time was spent in each heart-rate zone:
...
Based on my analysis in the previous step, I present the following findings from the data set. The full R code used for this section is included in Appendix E.
...
```{r share-plot-avg-time-by-intensity, echo=FALSE, fig.dim = c(6, 4)}
print(viz_avg_time_by_intensity)
```
...
Visualising users' Very Active Time as a percentage of their Active Time produces an approximately symmetrical, bimodal distribution, showing no clear preference for low- or high-intensity activity in the cohort:
...
> Users get 7500 steps in per day on average
...
I found a strong correlation between time spent exercising and distance covered, which I concluded indicated users prefer mobile exercise; if this was not true and users had no preference, then there would be no correlation as high-volume exercisers would be just as likely to log low distances as high distances.
...
The majority of users log sleep on a small minority of nights, with a long tail of users who log an increasing majority of nights:

```{r share-plot-night-usage-sleep-logs, echo=FALSE}
print(viz_night_usage_sleep_logs)
```

Supplementary analysis of those users who logged heart-rate data shows a similar trend for overnight heart-rate logging:

```{r share-plot-sleep-logs-vs-hr-logs, echo=FALSE}
print(viz_sleep_logs_vs_hr_logs)
```

> Most users logged heart rate data during the day, but not during the night

```{r share-plot-heartrate-usage, echo=FALSE}
print(viz_daytime_heartrate_usage)
print(viz_nighttime_heartrate_usage)
```

> The majority of heart rate data was not logged during high-intensity activity

```{r share-plot-heartrate-intensities, echo=FALSE}
print(viz_heartrate_intensities)
```

> Users get an appropriate amount of good-quality sleep

Users typically log around 7 hours sleep per night:

```{r share-plot-sleep-quantity, echo=FALSE}
print(viz_average_hours_asleep)
```

Almost all time logged is spent Asleep, the highest-quality sleep rank logged by the FitBits:

```{r share-plot-sleep-quality, echo=FALSE}
print(viz_average_pct_asleep)
```

> Users did not improve on any of the metrics analysed

The users in the cohort showed no evidence of increasing the following average metrics over time:
...
For all of these metrics, average values typically remained the same or decreased slightly
...
```{r share-plot-sleep-quantity-over-time, echo=FALSE}
print(viz_sleep_quantity_vs_time)
```

```{r share-plot-sleep-quality-over-time, echo=FALSE}
print(viz_sleep_quantity_vs_time)
```
...
<div style="text-align: center;">
  ![Bellabeat Ivy on a woman's wrist](imgs/bellabeat_ivy_on_wrist_cropped.png)<br>
</div>

To mitigate these potential consumer concerns, the marketing should highlight the hypo-allergenic and hygienic aspects of the materials used in the Ivy, positioning these as keeping users comfortable during overnight wear.
...
Users were found to preference mobile activities, with very little evidence of high-intensity, low-distance exercise in the data set. Marketing materials should therefore focus on mobile, outdoor activities like light jogging, walking dogs, and cycling, instead of stationary activities like gym workouts.
...
# Print code inside a given code chunk with back-tick header/footer
...
```{r ref.label="vrfy-get-intensity-range", echo=TRUE, eval=FALSE}
```

```{r vrfy-print-intensity-range, results='markdown'}
print(unique_intensities)
```
...
```{r ref.label="vrfy-compare-intensity-data", echo=TRUE, eval=FALSE}
```

```{r vrfy-print-comp-intensity-data, results='markdown'}
glimpse(intensity_daily_comp)
```
...
Activity trackers like FitBits detect activity intensity partly by measuring the user's heart rate while exercising: a higher heart rate corresponds with a higher degree of exertion. I looked up the product manuals for all heart-rate compatible FitBits in the data set: these confirmed that each model broke down user activity into four default heart-rate zones:
...
```{r ref.label="vrfy-get-sleep-value-range", echo=TRUE, eval=FALSE}
```

```{r vrfy-print-sleep-value-range, results='markdown'}
print(unique_sleep_values)
```
...
```{r ref.label="vrfy-compare-sleep-value-data", echo=TRUE, eval=FALSE}
```

```{r vrfy-print-comp-sleep-value-data, results='markdown'}
glimpse(sleepDay_comp)
```
...
## Appendix C: "Process" Stage Code

```{r process-apdx-labels, include=TRUE}
process_apdx_labs <- grep("^process-", knitr::all_labels(), value = TRUE)
```

```{r process-apdx-code, ref.label=process_apdx_labs, echo = TRUE, include = TRUE, eval=FALSE}
```

## Appendix D: "Analyse" Stage Code
