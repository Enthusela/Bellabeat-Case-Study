...
* How much exercise are people doing? OR: Frequency of exercise: do those who track track often? 
* Average intensity: are people tracking their exercises exercising very hard? Answer: it's 50/50
* What kind of exercise are people doing?
  * Intensity vs distance: who's outdoors, and who's at the gym?
* Do people change their exercise habits over time?

Findings:
* All users are majority "Lightly Active" when active at all

Not clear yet:
* What level of Very Active do most users fall below?
* When people work out, are they "Very Active"? There appears to be a mix of Fairly and Very active levels, but this is aggregated across the week and may not reflect workouts: for example, it could be the case that Fairly Active minutes accrue during the week similarly to Lightly Active minutes, whereas Very Active minutes come in bursts
* Is "Fairly Active" more common than "Very Active"?

#### How much exercise are people doing?

Question: Simple questions first: how much time are people spending in a non-sedentary state?
...
Answer: The majority of users (24, or 72.7%) get less than 45 minutes of Active Time per week on average.

Findings:
* Most people in the dataset are not doing much more than very light exercise.
* There are a small number (6, or 18%) who do more than 1 hour a week on average.

Recommendations:

* Based on this data, the number of people using their fitbits to track intense activity is quite small. The marketing strategy should focus on other features.

#### What kind of exercise are people doing?

Question: Do people vary in their exercise intensity? When exercising, i.e. not Sedentary or Lightly Active, do users log more "Fairly Active" or "Very Active" time? 

```{r viz_proportion_intensity}
mean_daily_intensities_active <- mean_daily_intensities %>%
  filter(intensity != "sedentary" & intensity != "lightly_active")

mean_daily_intensities_wide <- mean_daily_intensities_wide %>%
  mutate(total_active_time = fairly_active + very_active) %>%
  mutate(proportion_very_active = very_active / total_active_time)

mean_daily_intensities_wide <- mean_daily_intensities_wide %>%
  arrange(proportion_very_active)

mean_daily_intensities_active$id <- factor(
  mean_daily_intensities_active$id,
  levels = mean_daily_intensities_wide$id)

ggplot(mean_daily_intensities_active, aes(x= id, y= mean_minutes, fill= intensity)) +
  geom_bar(stat = "identity",
           position = "fill") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
Answer: It's quite evenly spread out from around 10% Very Active to around 90% Very Active across all users, with no clear outliers.

Question: Do people who track more do more-intense exercise?
...
plot_all_data <- ggplot(mean_daily_intensities_wide, aes(x=total_active_time, y=proportion_very_active)) +
  geom_point() +
...
mdiw_temp <- mean_daily_intensities_wide %>%
  filter(total_active_time <= 1.25) # id != "3977333714")

plot_no_upper_quint <- ggplot(mdiw_temp, aes(x=total_active_time, y=proportion_very_active)) +
  geom_point() +
  geom_smooth(method = "lm",
              se = FALSE,
              color = "blue") +
...
mdiw_temp <- mean_daily_intensities_wide %>%
  filter(total_active_time > 0.25) # id != "3977333714")

plot_no_lower_quint <- ggplot(mdiw_temp, aes(x=total_active_time, y=proportion_very_active)) +
  geom_point() +
  geom_smooth(method = "lm",
              se = FALSE,
              color = "blue") +
...
Findings: 
* Overall, Very Active Proportion was positively correlated with Active time, with an R^2 value of 0.27.
* Removing the upper quintile decreased the correlation to 0.23, indicating that those doing particularly high levels of exercise are also particularly active.
* Removing the lower quintile increased the correlation to 0.32, indicationg that those doing the least exercise are also engaged in a wider range of activities.

Overall, these findings indicate that the majority of the cohort is engaged in a wide range of activities, with those doing more exercise being slightly more likely to engage in more intense exercise. There is a large subset of the group doing a low amount of varied exercise, as well as a small group of outliers doing a higher amount of more-intense exercise.
...
Question: How many steps are people getting?
...
ggplot(average_daily_steps) +
  ...
  labs(title = "Average Daily Steps by User",
       ...
       y = "Users")
```
```{r waste_of_time_plots, eval=FALSE}
ggplot() +
...
p <- ggplot(average_daily_steps, aes(x = average_steps)) +
...
p + geom_density(color = "red", size = 1) +
  scale_y_continuous(
    sec.axis = sec_axis(~ . / sum(average_daily_steps$average_steps)),
    name = "Density"
    ) +
  labs(title = "Average Daily Steps by User",
       x = "Average Daily Steps",
...
Question: What's the most popular day to step?

```{r average_steps_by_weekday}
average_weekday_steps <- steps_sum_days_tall %>%
  mutate(weekday = wday(activity_day)) %>%
  group_by(weekday) %>%
  summarize(weekday_average = mean(steps_total))

ggplot(average_weekday_steps) +
  geom_col(aes(x=weekday,y=weekday_average))
```
...
Answer: Mostly no. The majority of users actually did fewer steps over time
What this tells us: 

* You shouldn't claim that this device encourages people to walk more.
...
* Check when people are wearing their fitbits
  ...
  * Need to know if they're wearing them first thing in the morning
...
View(nights_logged_by_user)
```

### Heart Rate Tracking

"Use it to track your workout progress and optimize personal training routines."
