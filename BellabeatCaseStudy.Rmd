...
### TODO: Heart Rate Tracking and Activity Tracking
...
* From the Surge Manual: [wearing your tracker 24/7 does not allow your skin to breathe](https://myhelp.fitbit.com/resource/manual_surge_en_US): This may be part of the reason people don't track overnight, and needs to be considered when marketing the features that require overnight wear

### TODO: Resting Heart Rate and Cardiac Coherence

TODO: Re-implement my heart-rate log tracking here to determine how much daytime/nighttime logging is being done
...
This will help me understand whether or not the RHR and CC functions will appeal to the user base: for instance, users who rarely wear their existing heart-rate monitors, or only wear them during intense exercise, are perhaps less likely to care about their longer-term cardiac performance.

While I could analyse the heart-rate monitor data directly, the activity_sum_days_wide data frame has done a lot of the work for me already: The Intensity data in the table is based on heart-rate zones, so any minutes logged in that data frame are minutes where the user had their heart-rate monitor on.

First question is how often people track their heart-rate:

```{r overall_heart_rate_tracking}
...
* A majority of users (54% or 18 users) log 22 to 24 hours a day on average
* The remaining 46% of users log between 15 and 19 hours per day
* We know from previous analysis of sleep logs that most users do not log overnight

* Actually, this kind of contradicts that, hey
* I'll figure it out tomorrow, it's getting dark and a bit shit up here
...
plot_histo_pareto(sleep_counts$pct_logged_asleep, bin_width = 0.1) +
  labs(title = "Count of Nights Logged Asleep")
plot_histo_pareto(sleep_counts$pct_logged_in_bed, bin_width = 0.1) +
  labs(title = "Count of Nights Logged In Bed")

ggplot(sleep_counts) +
  geom_boxplot(aes(y = pct_logged_asleep)) + 
  scale_y_continuous(breaks = seq(0, 33, by = 0.1))

```

Findings:

* Frequency of sleep logging varied considerably across the user group
* 27% of users (9) logging sleep on 20% of nights or less: including the nine users that did not log any sleep records, this figure jumps to 54% (18 users).
* 30% of users logged sleep on 70% of nights or more
* The FitBit devices available at the time the dataset was recorded all have sleep-tracking functionality, so these findings indicate that the majority of users are not making use of this function.
* One possible reason for this is indicated by the manual for the FitBit Surge, which informs the user that "[wearing your tracker 24/7 does not allow your skin to breathe](https://myhelp.fitbit.com/resource/manual_surge_en_US)". The FitBit surge uses a silicon strap, similar to the Bellabeat Ivy

#### Recommendations

* The Readiness Score can be marketed as a value-add feature that produces more useful information when worn to bed than competing products
...
Several functions and features of the Ivy are not present on the FitBits in the data set. These features include:

* Cardiac Coherence tracking
* Resting Heart Rate
* Respiratory Rate
* Hydration tracking
...
Given the lack of data, it's not possible to directly analyse existing user behaviours with respect to these features.
