...
* Methods for storing the details of my analysis methodology separate from the data itself, e.g. separate source code files, but not macros stored within spreadsheet files
...
for (name in df_names) {
    cat("\n", "colnames(", name, ") Unclean:\n", sep = "")
    print(colnames(get(name)))
    
    assign(name, get(name) %>% clean_names())
    
    cat("\n", "colnames(", name, ") Cleaned:\n", sep = "")
    print(colnames(get(name)))
}
```

``` {r update_variable_names}
# Create a data frame containing all of the replacements to be made
...
# WARNING: changes without a specified table will ALWAYS be applied to matching column names, even if this interferes with a table-specific change
renames <- renames %>%
    rbind(.,data.frame(old = "activity_date",              new = "date",                       table = "")) %>%
    rbind(.,data.frame(old = "activity_day",               new = "date",                       table = "")) %>%
    rbind(.,data.frame(old = "fairly_active_distance",     new = "distance_fairly_active",     table = "")) %>%
    rbind(.,data.frame(old = "fairly_active_minutes",      new = "minutes_fairly_active",      table = "")) %>%
    rbind(.,data.frame(old = "light_active_distance",      new = "distance_lightly_active",    table = "")) %>%
    rbind(.,data.frame(old = "light_active_minutes",       new = "minutes_lightly_active",     table = "")) %>%
    rbind(.,data.frame(old = "lightly_active_distance",    new = "distance_lightly_active",    table = "")) %>%
    rbind(.,data.frame(old = "lightly_active_minutes",     new = "minutes_lightly_active",     table = "")) %>%
    rbind(.,data.frame(old = "logged_activities_distance", new = "distance_logged_activities", table = "")) %>%
    rbind(.,data.frame(old = "me_ts",                      new = "mets",                       table = "")) %>%
    rbind(.,data.frame(old = "sedentary_active_distance",  new = "distance_sedentary",         table = "")) %>%
    rbind(.,data.frame(old = "sedentary_distance",         new = "distance_sedentary",         table = "")) %>%
    rbind(.,data.frame(old = "sedentary_active_minutes",   new = "minutes_sedentary",          table = "")) %>%
    rbind(.,data.frame(old = "sedentary_minutes",          new = "minutes_sedentary",          table = "")) %>%
    rbind(.,data.frame(old = "sleep_day",                  new = "date",                       table = "")) %>%
    rbind(.,data.frame(old = "step_total",                 new = "steps_total",                table = "")) %>%
    rbind(.,data.frame(old = "total_distance",             new = "distance_total",             table = "")) %>%
    rbind(.,data.frame(old = "total_intensity",            new = "intensity_total",            table = "")) %>%
    rbind(.,data.frame(old = "total_minutes_asleep",       new = "sleep_minutes_asleep_total", table = "")) %>%
    rbind(.,data.frame(old = "total_sleep_records",        new = "sleep_records_total",        table = "")) %>%
    rbind(.,data.frame(old = "total_time_in_bed",          new = "sleep_minutes_bed_total",    table = "")) %>%
    rbind(.,data.frame(old = "tracker_distance",           new = "distance_tracker",           table = "")) %>%
    rbind(.,data.frame(old = "value",                      new = "heart_rate", table = "heartrate_src_seconds_tall")) %>%
    rbind(.,data.frame(old = "value",                      new = "sleep_rank", table = "sleep_src_mins_tall")) %>%
    rbind(.,data.frame(old = "very_active_distance",       new = "distance_very_active",       table = "")) %>%
    rbind(.,data.frame(old = "very_active_minutes",        new = "minutes_very_active",        table = ""))
print(renames)
...
* factor variables replaced with factor=n:m format


``` {r update_variable_types_setup echo = FALSE}
...
retypes <- retypes %>%
  rbind(., data.frame(name = "id",              new_type = "chr")) %>%
  rbind(., data.frame(name = "log_id",          new_type = "chr")) %>%
  ...
  for (i in 1:nrow(retypes)) {
    ...
    if (col_name %in% colnames(df)) {
      if (col_type_new == "datetime") {
        cat("INFO: Converting ", df_name, "$", col_name, " to datetime... \n", sep = "")
        ...
        cat("INFO: Converting ", df_name, "$", col_name, " to character \n", sep = "")
